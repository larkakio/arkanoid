...
export function GameCanvas() {
  ...
  useEffect(() => {
    ...
    const gameLoop = (time: number) => {
      ...
      const newBalls = currentBalls.map((ball) => {
        ...
        if (
          y + BALL_RADIUS >= py &&
          y - BALL_RADIUS <= py + PADDLE_HEIGHT &&
          x >= px - pw / 2 &&
          x <= px + pw / 2
        ) {
            ...
            if (brick.hits >= brick.hitsRequired) {
              for (let i = 0; i < 8; i++) {
                addParticle({
                  ...
                  life: 1,
                  color: BRICK_COLORS[brick.type] || "#fff",
                  size: 4,
                });
              }
              if (Math.random() < 0.2) {
                const types = ["enlarge", "laser", "slow", "catch", "disruption"];
                const type = types[Math.floor(Math.random() * types.length)] as PowerUpType;
                setPowerUps((p) => [
                  ...p,
                  {
                    x: brick.x + bw / 2 - POWERUP_SIZE / 2,
                    y: brick.y,
                    type,
                    width: POWERUP_SIZE,
                    height: POWERUP_SIZE,
      ...
      const paddleState = useGameStore.getState();
      const pady = GAME_HEIGHT - PADDLE_HEIGHT - 10;
      const padx = paddleState.paddleX;
      const padw = paddleState.paddleWidth;
      const newPowerUps = statePowerUps
        .map((p) => ({ ...p, y: p.y + 2 }))
        .filter((p) => {
          if (p.y > GAME_HEIGHT) return false;
          if (
            p.y + p.height >= pady &&
            p.y <= pady + PADDLE_HEIGHT &&
            p.x + p.width >= padx - padw / 2 &&
            p.x <= padx + padw / 2
          ) {
            addScore(1000);
            applyPowerUp(p.type);
      ...
      if (newBricks.length === 0) {
        addScore(1000);
        nextLevel();
        if (useGameStore.getState().level > LEVELS.length) {
  ...
  return (
    <div
      ...
      className="game-container touch-none select-none relative mx-auto"
    ...
    >
      <canvas
        ...
        onClick={() => ballStuck && releaseBall()}
      />
      {ballStuck && (
        <div
          className="absolute inset-0 flex items-center justify-center bg-black/30 rounded-lg"
          onClick={() => useGameStore.getState().releaseBall()}
        >
          <p className="text-neon-cyan font-orbitron text-lg">Tap to launch</p>
        </div>
      )}
      {isPaused && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 rounded-lg">
          <p className="text-white font-orbitron text-2xl">PAUSED</p>
        </div>
      )}
    </div>
  );
